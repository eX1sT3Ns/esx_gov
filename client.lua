	...
	while true do
		...
		if(isNear(coord1)) then
			if(playerJob == "gouv") then
				Info("Apasa ~INPUT_PICKUP~ pentru a urca la etaj.")
				...
				Info("Apasa ~INPUT_PICKUP~ pentru sonnerie.")
		...
		if(isNear(coord2)) then
			Info("Apasa ~INPUT_PICKUP~ pentru a cobora la parter.")
...
function renderMenu(name, menuName)
	...
	local elements = {}
  	...
  	table.insert(elements, {label = 'Scoate banii ', value = 'withdraw_society_money'})
  	table.insert(elements, {label = 'Depoziteaza Banii',        value = 'deposit_money'})
			...
			if data.current.value == 'withdraw_society_money' then
				...
				ESX.UI.Menu.Open(
					...
					function(data, menu)
						...
						if amount == nil then
							ESX.ShowNotification('Suma invalida')
						else
							menu.close()
							print(_name)
...
AddEventHandler("gouv:sendRequest", function(name,id)
	stopRequest = true
	SendNotification("~b~"..name.." ~w~ a suna la usa guvernului !")
	SendNotification("~INPUT_ENTER~ pentru ~g~acceptare~w~ / ~INPUT_DETONATE~ pentru ~r~refuz~w~.")
...
AddEventHandler("gouv:sendStatus", function(status)
	if(status == 1) then
		SendNotification("~g~Cineva a venit sa deschida usa !")
		SetEntityCoords(GetPlayerPed(-1),coord2.x,coord2.y,coord2.z)
	else
		SendNotification("~r~Nimeni nu a deschos usa.")
	end
end)
